...
ผู้เข้าสอบจะถูกลบก็ต่อเมื่อ **ไม่เหลือเกียรติบัตรและไม่มีอะไรในรอบอื่นชี้มาหา**
(ทั้งหน้าที่จับคู่ไว้ และรายชื่อผู้เข้าสอบของรอบอื่นที่ผูกกับคนนี้)
เงื่อนไขหลังสำคัญมาก เพราะการตัดสินของแอดมินในรอบอื่นชี้มาที่ตัวคนนั้น
ถ้าลบทิ้งงานของรอบอื่นจะหลุดเป็นค่าว่างโดยไม่มีอะไรฟ้อง

บันทึกการแก้ไข (audit_events) ของรอบที่ถูกลบยังอยู่ต่อ พร้อมชื่อรอบกำกับไว้
"""
        ...
        rows = conn.execute(
            ...
            WHERE c.batch_id = %s
              ...
              AND NOT EXISTS (
                    ...
                    WHERE re.student_id = c.student_id AND re.batch_id <> %s)
            """,
